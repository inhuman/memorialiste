...
Поэтому манифест критичен: без per-doc `covers` каждый doc видел бы каждое
изменение, LLM тратила бы токены на отделение мух от котлет, а
user guide переписывался бы при каждой правке внутренней сантехники.
...
memorialiste менять не нужно — просто наводите `--provider-url` на прокси
и подстраиваете `--model`.
...
[one-api](https://github.com/songquanpeng/one-api) — агрегатор с веб-UI,
такая же OpenAI-совместимая поверхность. Наводите `--provider-url` на
его базовый URL.

### Облако: OpenRouter

[OpenRouter](https://openrouter.ai) маршрутизирует к Claude, GPT-4,
...
Значение `--api-key` отправляется как `Authorization: Bearer <key>` к
провайдеру. Комбинируется с `--model-params` для настройки temperature,
top_p и пр.
...
`--repo-meta=extended` добавляет remote URL (токены вырезаны), ветку,
последние 5 тегов с датами — полезно для CHANGELOG / release-notes.
...
`--code-search` даёт LLM функцию `search_code` через function calling.
Прямо в процессе генерации модель может попросить любую Go-декларацию
в репозитории по regex-имени; tool возвращает тело функции/метода/типа
с путём и диапазоном строк. Полезно когда diff'а одного мало (например,
doc одного пакета упоминает символы из другого).
...
встраивает enclosing функцию вокруг каждой изменённой строки, поэтому
